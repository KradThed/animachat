 ...
 * Delegates connect with ?token=JWT&role=delegate&delegateId=xxx
 */
...
export function delegateWebsocketHandler(
  ...
  if (!token || !delegateId) {
    console.warn('[DelegateHandler] Missing token or delegateId');
    ws.close(1008, 'Missing token or delegateId');
    return;
  }
