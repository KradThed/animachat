<template>
  ...
  >
    ...
    />
      ...
      <!-- Tool call blocks (if present) -->
      <div v-if="toolPairs.length > 0 && !isEditing" class="tool-calls-section mb-2">
        <div v-for="(pair, idx) in toolPairs" :key="'tool-' + idx" class="tool-call-card">
          <div class="tool-call-header" @click="toggleTool(idx)">
            <v-icon size="16" class="mr-2"
              :color="pair.result ? (pair.result.is_error || pair.result.isError ? 'error' : 'success') : 'warning'">
              {{ pair.result
                ? (pair.result.is_error || pair.result.isError ? 'mdi-alert-circle' : 'mdi-check-circle')
                : 'mdi-loading mdi-spin' }}
            </v-icon>
            <span class="tool-name">{{ pair.call.name }}</span>
            <v-spacer />
            <v-icon size="14" :style="{ transform: expandedTools.has(idx) ? 'rotate(180deg)' : '', transition: 'transform 0.2s' }">
              mdi-chevron-down
            </v-icon>
          </div>
          <div v-if="expandedTools.has(idx)" class="tool-call-details">
            <div class="tool-label">Input</div>
            <pre class="tool-json">{{ JSON.stringify(pair.call.input, null, 2) }}</pre>
            <template v-if="pair.result">
              <div class="tool-label mt-2">Result</div>
              <pre class="tool-result" :class="{ 'tool-error': pair.result.is_error || pair.result.isError }">{{ typeof pair.result.content === 'string' ? pair.result.content : JSON.stringify(pair.result.content, null, 2) }}</pre>
            </template>
          </div>
        </div>
      </div>
...
// Extract tool_use/tool_result blocks, paired together
const toolPairs = computed(() => {
  const branch = currentBranch.value;
  if (!branch.contentBlocks || branch.contentBlocks.length === 0) return [];

  const blocks = branch.contentBlocks;
  const resultMap = new Map<string, any>();
  for (const b of blocks) {
    if ((b as any).type === 'tool_result') {
      const id = (b as any).tool_use_id || (b as any).toolUseId;
      if (id) resultMap.set(id, b);
    }
  }

  const pairs: Array<{ call: any; result: any | null }> = [];
  for (const b of blocks) {
    if ((b as any).type === 'tool_use') {
      pairs.push({ call: b, result: resultMap.get((b as any).id) || null });
    }
  }
  return pairs;
});
...
.tool-calls-section .tool-call-header {
  display: flex;
  align-items: center;
  padding: 6px 12px;
  cursor: pointer;
...
.tool-calls-section .tool-call-header:hover {
  background: rgba(var(--v-theme-on-surface), 0.06);
}
.tool-calls-section .tool-name {
  font-weight: 500;
  font-size: 0.82rem;
}
.tool-calls-section .tool-call-details {
  padding: 8px 12px;
  border-top: 1px solid rgba(var(--v-theme-on-surface), 0.08);
}
.tool-calls-section .tool-label {
  font-size: 0.75rem;
  color: rgba(var(--v-theme-on-surface), 0.5);
  margin-bottom: 4px;
}
.tool-calls-section .tool-json,
.tool-calls-section .tool-result {
